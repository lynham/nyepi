...
#library(s2)
#world <- st_as_sf(s2_data_tbl_countries)

world <- ne_countries(scale = "medium", returnclass = "sf")

library("ggspatial")
#world_points<- st_centroid(world, check=FALSE)



#world_points <- cbind(world, st_coordinates(st_centroid(world$geometry)))

bali<-data.frame(longitude=115.091831,latitude=-8.341536)
```
...
library(tidyverse)
library(reshape)
#javabalinusatenggara <- read_excel("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/tidy_javabalinusatenggara_provinces.xlsx")

javabalinusatenggara <- read_excel("/Volumes/GoogleDrive/My Drive/Nyepi/Processing Scripts/VNP46A1provinces.xlsx")
...
```{r include=FALSE}
library(readxl)
#javabalinusatenggaracounties <- read_excel("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/tidy_javabalinusatenggara_counties.xlsx")
...
# javabalinusatenggaracounties$days_from <- ifelse(javabalinusatenggaracounties$year==2017,as.Date(javabalinusatenggaracounties$newdate) - as.Date("2017-03-28"),NA)
# 
# javabalinusatenggaracounties$days_from <- ifelse(javabalinusatenggaracounties$year==2018,as.Date(javabalinusatenggaracounties$newdate) - as.Date("2018-03-17"),javabalinusatenggaracounties$days_from)
# 
# javabalinusatenggaracounties$days_from <- ifelse(javabalinusatenggaracounties$year==2019,as.Date(javabalinusatenggaracounties$newdate) - as.Date("2019-03-07"),javabalinusatenggaracounties$days_from)
# 
# javabalinusatenggaracounties$days_from <- ifelse(javabalinusatenggaracounties$year==2020,as.Date(javabalinusatenggaracounties$newdate) - as.Date("2020-03-25"),javabalinusatenggaracounties$days_from)
...
#clist <- read_csv("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/county_list.csv", 
 #   col_names = FALSE)
  
clist <-  readLines("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/county_list_new.csv")
...
stargazer(list(regList), title="Percent Change in Total Radiance for Counties (Regencies) in Bali. Each column estimates the effect of Nyepi for a different regency (indicated in the column title). Estimated regression coefficients are displayed with their associated standard errors underneath in parentheses. Asterisks indicate the results of individual two-sided tests of statistical significance: $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01.",
   ...
   #       keep=c("nyepi","county","nyepi*county"),
...
religionbycounty <- read_excel("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/religionbycounty.xlsx")
...
clist3 <-  readLines("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/county_list2.csv")
...
# This code makes a dataframe with county names and % reduction in lights
...
```{r include=FALSE}
# Check the issue with Menddoyo district
...
```{r eval=FALSE, include=FALSE}
library(plm)
religionbyzipcode <- read_excel("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/religionbyzipcode_clean.xlsx")
...
#zlist <-  readLines("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/zipcode_list.csv")
load("zipcode_predictions_newALLDAYS.RData")
# This code makes a dataframe with county names and % reduction in lights
#coeffdf2 <- data.frame(zlist,percentchangeinlight2)
#library(data.table)
#setnames(coeffdf2, "zlist", "zipcodename")
...
indonesiazipcodes <- read_sf(dsn="/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/IDN_adm3",layer="gadm36_IDN_3") # Is this important?
...
coordinates <- read_csv("/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/coordinates.csv")
...
```{r message=FALSE, warning=FALSE, include=FALSE}
library(gridExtra)
indonesiacounties <- read_sf(dsn="/Volumes/GoogleDrive/My Drive/Nyepi/Alemarie/IDN_adm2",layer="gadm36_IDN_2")
...
in Radiance",barwidth = 8,frame.colour = "black"))+
  # scale_fill_gradient2(high = "#d95f0e", low = "#ffffe5", midpoint=0.1,
  #                     guide = "colorbar",labels =scales::percent_format(accuracy = 5L))+
    # scale_fill_gradientn(colours = c("#ffffe5","#FFF6D9","#FAC192","#F4A770","#d95f0e"),
    #                      values = c(0,0.1713595,0.4840651,0.7038904,1),
                             scale_fill_gradientn(colours = #c("#ffffe5","#ffffe5","#FAC192","#F4A770","#d95f0e"),
                                                    c("#ffffe5","#ffffe5","#ffffe5","#d95f0e","#d95f0e"),
                        # values = c(0,0.45,0.4840651,0.7038904,1),
  ...
  #                      guide = "colorbar",labels =scales::percent_format(accuracy = 5L))+
    ...
    #                      values = c(0,0.1713595,0.4840651,0.7038904,1),
                            scale_fill_gradientn(colours = c("#ffffe5","#ffffe5","#ffffe5","#F4A770","#d95f0e"),
                                             #       c("#ffffe5","#ffffe5","#ffffe5","#F4A770","#d95f0e"),
                        values = c(0,0.4,0.7,0.8,1),
                      #  values = c(0,0.6,0.69,0.7,1), 
                      guide = "colorbar",labels =scales::percent_format(accuracy = 5L))+
  theme(panel.grid.major = element_line(
    color = gray(.5), linetype = "dashed", size = 0.5), 
    panel.background = element_rect(fill = "aliceblue",color = "black"),
